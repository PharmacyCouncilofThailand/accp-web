      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          if (data.error === "ACCOUNT_PENDING") {
            ...
            // Store email and reason for redirect to resubmit page
            const reason = data.rejectionReason || "";
            const resubmitUrl = `/${locale}/resubmit-document?email=${encodeURIComponent(email)}&reason=${encodeURIComponent(reason)}`;
            window.location.href = resubmitUrl;
            return;
          } else {
            setError(
              locale === "th"
          ...
          setError(
            data.error ||
              (locale === "th" ? "เข้าสู่ระบบไม่สำเร็จ" : "Login failed"),
          );
        }
        setIsLoading(false);
        return;
